...
public class FamilyCommandServiceTest {
	...
	@Test
	public void testFamilyImportCommand() {
		
		File orderStatus = new File("src/test/java/05_family.xml");
		try {
			...
			if (imports != null) {
				...
				for (ImportCommand importCommand : commands) {
					Boolean actuvalResulut = familyCommandService.importCommand(importCommand, marketLive, config,
							catalogModel);
