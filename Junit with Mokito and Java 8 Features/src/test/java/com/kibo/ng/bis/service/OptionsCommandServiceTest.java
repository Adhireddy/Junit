...
public class OptionsCommandServiceTest {
	...
	@Test
	public void testOptionsImportCommand() {
		
		File orderStatus = new File("src/test/java/03_options.xml");
		try {
			...
			if (imports != null) {
				...
				for (ImportCommand importCommand : commands) {
					Boolean actuvalResulut = optionsCommandService.importCommand(importCommand, marketLive, config,
							catalogModel);
