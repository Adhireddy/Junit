...
import com.kibo.ng.bis.jaxb.Skus;
...
public class PricingCommandServiceTest {
	...
	@Test
	public void testPricingImportCommand() {
		
		File inventory = new File("src/test/java/08_pricing.xml");
		try {
		...
		if (imports != null) {
			...
			for (ImportCommand importCommand : commands) {
				Boolean actuvalResulut = pricingCommandService.importCommand(importCommand, marketLive,config, catalogModel);
