...
public class CategoryCommandServiceTest {
	...
	@Test
	public void testCategoryimportCommand() {

		File orderStatus = new File("src/test/java/01_category.xml");
		try {
			...
			if (imports != null) {
				...
				for (ImportCommand importCommand : commands) {
					Boolean actuvalResulut = categoryCommandService.importCommand(importCommand, marketLive, config,
							catalogModel);
