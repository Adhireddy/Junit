...
public class OrderCommandServiceTest {
	...
	@Test
	public void testOrderImportCommand() {
		
		File order = new File("src/test/java/10_orderImport.xml");
		try {
			JAXBContext jaxbContext = JAXBContext.newInstance(Marketlive.class);
			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			Marketlive marketLive = (Marketlive) jaxbUnmarshaller.unmarshal(order);
			...
			if (imports != null) {
				...
				for (ImportCommand importCommand : commands) {
					Boolean actuvalResulut = orderCommandService.importCommand(importCommand, marketLive, config,
							catalogModel);
	...
	@Test
	public void testOrderStatusImportCommand() {
		
		File orderStatus = new File("src/test/java/11_orderStatusImport.xml");
		try {
			JAXBContext jaxbContext = JAXBContext.newInstance(Marketlive.class);
			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			...
			if (imports != null) {
				List<ImportCommand> commands = imports.getCommand();
				for (ImportCommand importCommand : commands) {
					Boolean actuvalResulut = orderStatusCommandService.importCommand(importCommand, marketLive, config,
							catalogModel);
					Boolean expectedResult = true;
					Assert.assertEquals(expectedResult, actuvalResulut);
				}
